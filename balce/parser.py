...
def toSubscript(n: int | float | str)-> str :
...
def formatEle(ele, form=bct.default_style)-> str :
	'''单个化学式unicode上下标规范与普通规范的转换
	ele: str - 要转换的化学式
	form=uni: nor or uni - nor转成普通规范, uni转成unicode上下标规范
	'''
	ele = \
	...
	if form is CStyle.ascii :
		return _COEFFICIENT_FORMAT_UNI.sub(
			lambda n : sreSubscript(n.group()),
			_EXPONENT_FORMAT_UNI.sub(
				lambda n : ('^%s ' if 0==2 else '^(%s)') % sreSuperscript(n.group()), # emmm... both can be identified
				ele))
	
	elif form is CStyle.unicode :
...
def formatCE(formula, form=bct.default_style, compact=False) :
	'''化学方程式unicode上下标式规范与普通规范的转换
	formula: str - 要转换的方程式
	form: CStyle - ascii转成普通规范, unicode上下标规范
	...
	if compact :
		return ' '.join(
			(formatEle(l, form), conf, formatEle(r, form))
		)
	else :
		left, right = \
		...
		if form is CStyle.ascii :
			left, right = \
				formatEle(left, form), formatEle(right, form)
